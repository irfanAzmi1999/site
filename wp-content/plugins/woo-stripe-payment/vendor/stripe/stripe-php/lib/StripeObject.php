...
class StripeObject implements \ArrayAccess, \Countable, \JsonSerializable
{
    ...
    {
        list($id, $this->_retrieveOptions) = \PaymentPlugins\Vendor\Stripe\Util\Util::normalizeId($id);
        $this->_opts = \PaymentPlugins\Vendor\Stripe\Util\RequestOptions::parse($opts);
    ...
    public function __set($k, $v)
    {
        ...
        $this->_values[$k] = \PaymentPlugins\Vendor\Stripe\Util\Util::convertToStripeObject($v, $this->_opts);
    ...
    public function updateAttributes($values, $opts = null, $dirty = true, $apiMode = 'v1')
    {
        foreach ($values as $k => $v) {
            ...
            if ('metadata' === $k && \is_array($v)) {
                ...
                $this->_values[$k] = \PaymentPlugins\Vendor\Stripe\Util\Util::convertToStripeObject($v, $opts, $apiMode);
            }
            if ($dirty) {
                $this->dirtyValue($this->_values[$k]);
            }
    ...
    public function serializeParamsValue($value, $original, $unsaved, $force, $key = null)
    {
        ...
        if (\is_array($value)) {
            if (\PaymentPlugins\Vendor\Stripe\Util\Util::isList($value)) {
                ...
                return \PaymentPlugins\Vendor\Stripe\Util\Util::convertToStripeObject($value, $this->_opts)->serializeParameters();
            }
    ...
    public function toArray()
    {
        ...
        return \array_reduce(\array_keys($this->_values), function ($acc, $k) use ($maybeToArray) {
            ...
            $v = $this->_values[$k];
            if (\PaymentPlugins\Vendor\Stripe\Util\Util::isList($v)) {
