...
class WC_Stripe_Admin_Order_Metaboxes {
	...
	public static function add_meta_boxes( $post_type, $post ) {
		...
		if ( ( \PaymentPlugins\Stripe\Utilities\FeaturesUtil::is_custom_order_tables_enabled() && $post instanceof \WC_Order )
			...
			add_action( 'admin_enqueue_scripts', function () use ( $order ) {
	...
	public static function enqueue_scripts( $order ) {
		wp_enqueue_script( 'wc-stripe-admin-order-metabox' );
		...
		wp_localize_script(
			...
			array(
				...
				'routes'          => array(
					'charge_view'     => WC_Stripe_Rest_API::get_admin_endpoint( 'wc-stripe/v1/order-actions/charge-view' ),
					'capture'         => WC_Stripe_Rest_API::get_admin_endpoint( 'wc-stripe/v1/order-actions/capture' ),
					'void'            => WC_Stripe_Rest_API::get_admin_endpoint( 'wc-stripe/v1/order-actions/void' ),
					'pay'             => WC_Stripe_Rest_API::get_admin_endpoint( 'wc-stripe/v1/order-actions/pay' ),
					'payment_methods' => WC_Stripe_Rest_API::get_admin_endpoint( 'wc-stripe/v1/order-actions/customer-payment-methods' ),
				),
				'messages'        => array(
