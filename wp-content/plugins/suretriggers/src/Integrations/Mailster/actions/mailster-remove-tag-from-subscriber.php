<?php
/**
 * MailsterRemoveTagFromSubscriber.
 * php version 5.6
 *
 * @category MailsterRemoveTagFromSubscriber
 ...
 * @since    1.0.0
 */
class MailsterRemoveTagFromSubscriber extends AutomateAction {
	 ...
	 * @var string
	 */
	public $action = 'mailster_remove_tag_from_subscriber';
	...
	public function register( $actions ) {
		...
		$actions[ $this->integration ][ $this->action ] = [
			'label'    => __( 'Remove Tag from Subscriber', 'suretriggers' ),
			'action'   => $this->action,
	 ...
	 * Action listener.
	 *
	 * Uses `mailster( 'tags' )->unassign_subscriber()` (classes/tags.class.php)
	 * which fires `mailster_tag_removed` on completion.
	 *
	...
	public function _action_listener( $user_id, $automation_id, $fields, $selected_options ) {
		...
		foreach ( $tag_ids as $tag_id ) {
			mailster( 'tags' )->unassign_subscriber( $tag_id, absint( $subscriber->ID ) );
		}
...
MailsterRemoveTagFromSubscriber::get_instance();
