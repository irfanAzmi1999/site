<?php
/**
 * MailsterRemoveSubscriberFromList.
 * php version 5.6
 *
 * @category MailsterRemoveSubscriberFromList
 ...
 * @since    1.0.0
 */
class MailsterRemoveSubscriberFromList extends AutomateAction {
	 ...
	 * @var string
	 */
	public $action = 'mailster_remove_subscriber_from_list';
	...
	public function register( $actions ) {
		...
		$actions[ $this->integration ][ $this->action ] = [
			'label'    => __( 'Remove Subscriber from List', 'suretriggers' ),
			'action'   => $this->action,
	 ...
	 * Action listener.
	 *
	 * Uses `mailster( 'lists' )->unassign_subscriber()` (classes/lists.class.php)
	 * which fires `mailster_list_removed` on completion.
	 *
	...
	public function _action_listener( $user_id, $automation_id, $fields, $selected_options ) {
		...
		foreach ( $list_ids as $list_id ) {
			mailster( 'lists' )->unassign_subscriber( $list_id, absint( $subscriber->ID ) );
		}
...
MailsterRemoveSubscriberFromList::get_instance();
