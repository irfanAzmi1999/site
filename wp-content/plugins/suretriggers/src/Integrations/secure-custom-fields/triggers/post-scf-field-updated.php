...
if ( ! class_exists( 'PostScfFieldUpdated' ) ) :
	...
	class PostScfFieldUpdated {
		 ...
		 * @var string
		 */
		public $trigger = 'post_scf_field_updated';
		...
		public function trigger_listener( $meta_id, $post_id, $meta_key, $meta_value ) {
			...
			if ( 'updated_post_meta' === current_action() && '_edit_lock' !== $meta_key ) {
				...
				$post_data = $_POST; // phpcs:ignore WordPress.Security.NonceVerification.Missing -- nonce verified upstream by ACF before updated_post_meta fires
	 ...
	 */
	PostScfFieldUpdated::get_instance();
