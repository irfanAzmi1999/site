 ...
 * @since    1.0.0
 */
class ActivateSnippet extends AutomateAction {
	 ...
	 * @var string
	 */
	public $action = 'code_snippets_activate_snippet';
	...
	public function register( $actions ) {
		$actions[ $this->integration ][ $this->action ] = [
			'label'    => __( 'Activate a Snippet', 'suretriggers' ),
			'action'   => $this->action,
	...
	public function _action_listener( $user_id, $automation_id, $fields, $selected_options ) {
		if ( ! function_exists( 'Code_Snippets\activate_snippet' ) || ! function_exists( 'Code_Snippets\get_snippet' ) ) {
			return [
				'status'  => 'error',
		...
		$result = \Code_Snippets\activate_snippet( $snippet_id );

		if ( ! $result ) {
			return [
				'status'  => 'error',
				'message' => __( 'Failed to activate snippet with ID: ', 'suretriggers' ) . $snippet_id,
			];
		}
