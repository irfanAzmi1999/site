 ...
 * @since    1.0.0
 */
class UpdateCCTItem extends AutomateAction {
	 ...
	 * @var string
	 */
	public $action = 'jet_engine_update_cct_item';
	...
	public function register( $actions ) {
		$actions[ $this->integration ][ $this->action ] = [
			'label'    => __( 'Update CCT Item', 'suretriggers' ),
			'action'   => $this->action,
	...
	public function _action_listener( $user_id, $automation_id, $fields, $selected_options ) {
		...
		$item_id  = isset( $selected_options['cct_item_id'] ) ? absint( $selected_options['cct_item_id'] ) : 0;
		...
		if ( empty( $item_id ) ) {
			return [
				'status'  => 'error',
				'message' => __( 'CCT Item ID is required.', 'suretriggers' ),
			];
		}
		...
		$existing_item = $factory->db->get_item( $item_id );
		...
		if ( ! $existing_item ) {
			return [
				...
				'message' => sprintf(
					...
					__( 'CCT item with ID %d not found.', 'suretriggers' ),
					$item_id
				),
			];
		}
		...
		$updated_item = $factory->db->get_item( $item_id );

		if ( ! $updated_item ) {
			return [
				'status'  => 'error',
				'message' => __( 'Failed to retrieve updated CCT item.', 'suretriggers' ),
			];
		}
		...
		$item_array = is_object( $updated_item ) ? (array) $updated_item : $updated_item;
