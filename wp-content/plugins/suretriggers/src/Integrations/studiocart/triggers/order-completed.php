<?php
/**
 * StudiocartOrderCompleted.
 * php version 5.6
 *
 * @category StudiocartOrderCompleted
...
if ( ! class_exists( 'StudiocartOrderCompleted' ) ) :
	 ...
	 */
	class StudiocartOrderCompleted {
		 ...
		 * @var string
		 */
		public $trigger = 'studiocart_order_completed';
		...
		public function register( $triggers ) {
			$triggers[ $this->integration ][ $this->trigger ] = [
				'label'         => __( 'Order Completed', 'suretriggers' ),
				'action'        => $this->trigger,
		...
		public function trigger_listener( $status, $order_data, $order_type ) {
			if ( ! is_array( $order_data ) || 'main' !== $order_type ) {
				return;
			}

			if ( ! isset( $order_data['status'] ) || 'completed' !== $order_data['status'] ) {
				return;
			}
	 ...
	 */
	StudiocartOrderCompleted::get_instance();
